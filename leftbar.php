<nav class="ts-sidebar">
	<ul class="ts-sidebar-menu">
	<li class="ts-label">Main</li>
	<li><a href="dashboard.php"><i class="fa fa-dashboard"></i> Dashboard</a></li>

<li><a href="#"><i class="fa fa-files-o"></i> Brands</a>
	<ul>
	<li><a href="create-brand.php">Create Brand</a></li>
	<li><a href="manage-brands.php">Manage Brands</a></li>
	</ul>
	</li>

<li><a href="#"><i class="fa fa-sitemap"></i> Bicycles</a>
	<ul>
	<li><a href="post-avehical.php">Post Bicycle</a></li>
	<li><a href="manage-vehicles.php">Manage Bicycles</a></li>
	</ul>
	</li>
	
<li><a href="manage-bookings.php"><i class="fa fa-users"></i> Manage Bookings</a></li>

<li><a href="testimonials.php"><i class="fa fa-table"></i> Manage Testimonials</a></li>

<li><a href="manage-contactusquery.php"><i class="fa fa-desktop"></i> Manage Report Queries</a></li>

<li><a href="reg-users.php"><i class="fa fa-users"></i> Registered Users</a></li>


	</ul>
</nav>